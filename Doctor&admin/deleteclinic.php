...
Select CID:<?php
				...
				$clinic_result = $conn->query('select * from clinic order by City,Town ASC');
				?>
				<center>
				<select name="cid">
			...
			<button type="submit" name="Submit1">Delete by CID</button><br><br>
			<br>---------OR---------<br>
			<br><br>Select Name:<br><?php
				require_once('dbconfig.php');
				...
				while($row = $clinic_result->fetch_assoc()) {
				?>
				<option value="<?php echo $row["cid"]; ?>"><?php echo $row["name"].", ".$row["city"]; ?></option>
				<?php
					}
					}
				?><br>
